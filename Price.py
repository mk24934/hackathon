...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 10 15:50:40 2018
...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov  2 10:13:51 2018
...
# os.chdir(dname)
os.chdir('/Users/markmcavoy/Documents/Blockchain/mlh-localhost-tech-marketplace-master')
# os.getcwd()
...
while True:
    ...
    for x in tickers:
        ...
        put = []
        for i in range(0, 20):
            K = round(K_min + i * two_sigma)[x]
            S = S0[x]
            d1 = (np.log(S/K) + (Rf + sigma[x]**2/2)*t)/(sigma[x]*np.sqrt(t))
            d2 = d1 - sigma[x]*np.sqrt(t)
            c = S * stats.norm.cdf(d1) - stats.norm.cdf(d2)* K * np.exp(-Rf*t)
            p = stats.norm.cdf(-d2)* K * np.exp(-Rf*t) - S * stats.norm.cdf(-d1)
            K_values.append(K)
            call.append(c)
            put.append(p)
